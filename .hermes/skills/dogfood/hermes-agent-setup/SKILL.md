...
sudo hermes gateway install --system --run-as-user <user>   # Linux servers / headless VPS
sudo hermes gateway start --system                          # Start system service
sudo hermes gateway status --system                         # Check system service
```
...
Why:
- `hermes gateway start` under `su hermes` often fails with `systemctl --user` / D-Bus permission issues on servers.
- Root may not have `hermes` on PATH, so use the full binary path or manage the service directly with `systemctl`.

If the system service crashes immediately with a traceback mentioning `/root/.hermes/.env`, inspect the unit file:

```bash
grep HERMES_HOME /etc/systemd/system/hermes-gateway.service
```

It must point at the Hermes user's home, not root:
...
journalctl -u hermes-gateway -n 50 --no-pager -l
```

This pitfall can happen if the service was installed while root had `HERMES_HOME=/root/.hermes` in its environment, causing the wrong value to be baked into the unit file.
...
### Headless Linux server: `hermes gateway start` fails
If you see errors like:
- `Command 'hermes' not found` when running as root
- `Failed to connect to bus: Permission denied` after `su hermes`

then you're probably trying to use the user-level systemd service from a non-login shell on a headless server.

Recommended fix for VPS/dedicated servers:
...
Why:
- `--system` installs a boot-time system service, which is more reliable on servers
- `--run-as-user hermes` keeps the gateway running as the non-root `hermes` user
- `/home/hermes/.local/bin/hermes` avoids root PATH problems when Hermes is only installed for the `hermes` user

Avoid using plain `su hermes` + `hermes gateway start` on servers; that often lacks the proper user dbus/session setup for `systemctl --user`.

Another headless-server pitfall: if the system service starts and immediately crashes, check the journal:

```bash
journalctl -u hermes-gateway -n 100 --no-pager -l
```

If you see Hermes trying to read `/root/.hermes/.env`, the unit file likely captured the wrong `HERMES_HOME` during install. Inspect and fix:

```bash
grep HERMES_HOME /etc/systemd/system/hermes-gateway.service
# should be /home/hermes/.hermes, not /root/.hermes
systemctl daemon-reload
systemctl reset-failed hermes-gateway
systemctl restart hermes-gateway
journalctl -u hermes-gateway --since "2 minutes ago" --no-pager -l
```
...
If you really want user-service mode instead, use a real login shell:

```bash
sudo -iu hermes
hermes gateway status
loginctl enable-linger hermes   # if needed
```
